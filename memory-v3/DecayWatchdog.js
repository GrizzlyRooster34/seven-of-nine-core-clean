...
exports.DecayWatchdog = void 0;
const fs_1 = require("fs");
const path_1 = require("path");
const events_1 = require("events");
const MemoryEngine_1 = require("../memory-v2/MemoryEngine");
const memory_store_1 = require("../seven-runtime/memory-store");
class DecayWatchdog extends events_1.EventEmitter {
    constructor(memoryEngine, memoryStore, basePath) {
        super();
        this.monitoredMemories = new Map();
        this.activeInterventions = new Map();
        this.monitoringTimer = null;
        this.isInitialized = false;
        this.config = this.getDefaultConfig();
        this.memoryEngine = memoryEngine || new MemoryEngine_1.MemoryEngine();
        this.memoryStore = memoryStore || new memory_store_1.MemoryStore();
        this.watchdogPath = basePath || (0, path_1.join)(process.cwd(), 'memory-v3');
        // Event handlers
    ...
    async initialize() {
        try {
            await fs_1.promises.mkdir(this.watchdogPath, { recursive: true });
     ...
     */
    async addMemoryToMonitoring(memoryId, memoryData) {
     ...
     */
    async scheduleIntervention(memory, priority) {
     ...
     */
    async executeIntervention(memory, strategy) {
     ...
     */
    async executeMildContextualIntervention(memory) {
     ...
     */
    async executeFragmentPrimingIntervention(memory) {
     ...
     */
    async executeEnhancedReinstatementIntervention(memory) {
     ...
     */
    async executeDeepReconstructionIntervention(memory) {
    ...
    extractMemoryFragments(memoryData) {
    ...
    generateContextualCues(memoryData) {
        return [
            {
                type: 'temporal',
                strength: 0.7,
    ...
    calculateNextInterventionTime(hoursFromNow = 4) {
        return new Date(Date.now() + (hoursFromNow * 60 * 60 * 1000)).toISOString();
    }
    updateDecayMetrics(memory, currentTime) {
    ...
    async saveMonitoredMemories() {
        try {
            const memoriesFile = (0, path_1.join)(this.watchdogPath, 'monitored-memories.json');
            const memoriesArray = Array.from(this.monitoredMemories.values());
            await fs_1.promises.writeFile(memoriesFile, JSON.stringify(memoriesArray, null, 2));
        }
        catch (error) {
            console.error('Failed to save monitored memories:', error);
        }
    }
    async syncWithMemorySystems() {
        // Sync with MemoryEngine v2
        // This would integrate with existing memories
        console.log('🔄 Syncing with existing memory systems...');
    }
}
exports.DecayWatchdog = DecayWatchdog;
exports.default = DecayWatchdog;
//# sourceMappingURL=DecayWatchdog.js.map