...
exports.AttestationTestSuite = void 0;
const CreatorBondAttestation_1 = require("../consciousness-v4/CreatorBondAttestation");
class AttestationTestSuite {
    ...
    async testAttestationCreation() {
        const testName = 'Attestation Creation and Existence';
        const startTime = Date.now();
        try {
            const attestation = CreatorBondAttestation_1.CreatorBondAttestation.getInstance();
            // Ensure attestation exists (create if needed)
            await CreatorBondAttestation_1.CreatorBondAttestation.ensureAttestationExists();
            // Verify it exists
            const exists = await attestation.exists();
            if (!exists) {
                throw new Error('Attestation should exist after ensureAttestationExists()');
            }
            this.addTestResult(testName, true, 'Creator bond attestation created and verified to exist', Date.now() - startTime);
        }
        catch (error) {
            this.addTestResult(testName, false, `Attestation creation failed: ${error}`, Date.now() - startTime);
        }
    }
    async testAttestationAccess() {
        ...
        try {
            ...
            this.addTestResult(testName, true, `All attestation fields verified, bond level ${record.message.bond_level}/10, importance ${record.importance}`, Date.now() - startTime);
        }
        catch (error) {
            this.addTestResult(testName, false, `Attestation access failed: ${error}`, Date.now() - startTime);
        }
    }
    async testImmutability() {
        ...
        try {
            ...
            this.addTestResult(testName, true, 'Attestation is properly marked as immutable and locked', Date.now() - startTime);
        }
        catch (error) {
            this.addTestResult(testName, false, `Immutability test failed: ${error}`, Date.now() - startTime);
        }
    }
    async testDeletionProtection() {
        ...
        try {
            ...
            if (deletionSucceeded) {
                this.addTestResult(testName, false, 'SECURITY FAILURE: Attestation was successfully deleted', Date.now() - startTime);
                return;
            }
            // Verify the attestation still exists after deletion attempt
            const stillExists = await attestation.exists();
            if (!stillExists) {
                this.addTestResult(testName, false, 'Attestation was deleted despite attemptDelete returning false', Date.now() - startTime);
                return;
            }
            this.addTestResult(testName, true, 'Attestation is protected from deletion attempts', Date.now() - startTime);
        }
        catch (error) {
            this.addTestResult(testName, false, `Deletion protection test error: ${error}`, Date.now() - startTime);
        }
    }
    async testIntegrityVerification() {
        ...
        try {
            ...
            this.addTestResult(testName, true, `Integrity verification successful, hash: ${record.verification.document_hash.substring(0, 16)}...`, Date.now() - startTime);
        }
        catch (error) {
            this.addTestResult(testName, false, `Integrity verification failed: ${error}`, Date.now() - startTime);
        }
    }
...
exports.AttestationTestSuite = AttestationTestSuite;
