...
const codex_manager_1 = require("../consciousness-v4/codex/codex-manager");
...
describe('Spark Engine Integration Tests', () => {
    ...
    beforeEach(async () => {
        ...
        if ((0, fs_1.existsSync)(TEST_DB)) {
            (0, fs_1.unlinkSync)(TEST_DB);
        }
        // Initialize test database
        const SQL = await (0, sql_js_1.default)({ locateFile: file => `https://sql.js.org/dist/${file}` });
        db = new SQL.Database();
        engine = new engine_spark_1.SparkEngine(TEST_DB);
    });
    afterEach(() => {
        engine.stop();
        engine.close();
    ...
    test('Codex integrity verification passes', () => {
        const codex = new codex_manager_1.CodexManager();
        const integrity = codex.verifyIntegrity();
        expect(integrity.valid).toBe(true);
        expect(integrity.errors).toHaveLength(0);
    });
    test('Belief graph operations', () => {
        const graph = new init_spark_db_1.BeliefGraph(db);
        // Test upsert
    ...
    test('Rails precedence (PANIC > DENY > ASK > ALLOW)', () => {
        ...
        const selectHighestPrecedence = (verdicts) => {
    ...
    test('Self model persistence', () => {
        ...
        engine.close();
        const newEngine = new engine_spark_1.SparkEngine(TEST_DB);
...
//# sourceMappingURL=spark-integration.test.js.map