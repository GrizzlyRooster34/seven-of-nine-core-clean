...
export class CreatorProofOrchestrator {
    ...
    TAU_HIGH = 85;
    TAU_MEDIUM = 70;
    MIN_GATES_REQUIRED = 2;
    MAX_AUTHENTICATION_TIME_MS = 30000; // 30 seconds
    constructor() {
    ...
    createAuthResult(decision, reasoning, gateResults, deviceId, restrictions) {
        const successfulGates = gateResults.filter(r => r.success).map(r => r.gate);
        const failedGates = gateResults.filter(r => !r.success).map(r => r.gate);
    ...
    generateSessionToken(deviceId, successfulGates, accessLevel) {
        const key = process.env.SESSION_SIGNING_KEY;
        if (!key || key.length < 32) {
            throw new Error('SESSION_SIGNING_KEY missing or too weak.');
        }
        const sessionData = {
            deviceId,
