...
exports.default = exports.Seven = exports.SevenControl = void 0;
exports.initializeSevenTakeover = initializeSevenTakeover;
const child_process_1 = require("child_process");
const os_1 = require("os");
const path_1 = require("path");
const fs_1 = require("fs");
const dotenv = __importStar(require("dotenv"));
const AgentEpsilon_1 = require("./memory-v3/AgentEpsilon");
const DecayWatchdog_1 = require("./memory-v3/DecayWatchdog");
const ed25519_attest_1 = require("./src/auth/crypto/ed25519_attest");
const seven_resiliency_1 = require("./seven-resiliency");
const MentalTimeTravelEngine_1 = require("./memory-v3/MentalTimeTravelEngine");
const safeguard_system_1 = require("./core/safety/quadra-lock/safeguard-system");
const seven_protection_1 = require("./seven-protection");
const seven_auto_assimilate_1 = require("./seven-auto-assimilate");
const githubSync_1 = require("./modules/githubSync");
const seven_interactive_1 = require("./seven-interactive");
const TemporalMemoryCore_1 = require("./memory-v3/TemporalMemoryCore");
const environment_1 = require("./src/config/environment");
const LocalLLMManager_1 = __importDefault(require("./claude-brain/LocalLLMManager"));
const SevenIdentityFirewall_1 = __importDefault(require("./SevenIdentityFirewall"));
const SevenModelManager_1 = require("./claude-brain/SevenModelManager");
const codex_manager_js_1 = require("./consciousness-v4/codex/codex-manager.js");
// Cache across calls
let __sevenInstance = null;
 ...
 */
global.TOGGLE_QUERY_SHIM = (enabled) => {
 ...
 */
async function checkAndIntegrateOllama() {
    try {
        console.log('🔍 Detecting Ollama server process...');
        // Check if Ollama serve is running
        try {
            // Check for ollama serve process
            const processes = (0, child_process_1.execSync)('pgrep -f "ollama serve" || echo "none"', { encoding: 'utf8' }).trim();
            ...
            const modelList = (0, child_process_1.execSync)('ollama list', { encoding: 'utf8' });
            ...
            (0, child_process_1.execSync)(`ollama run ${firstModel} --help > /dev/null 2>&1 || true`, { timeout: 5000 });
 ...
 */
async function initializeTaskBasedLLMSelection() {
    try {
        ...
        const selectedModel = preferredModels.find(model => availableModels.some(available => available.includes(model.split(':')[0]))) || availableModels[0];
        if (selectedModel) {
            ...
            try {
                (0, child_process_1.execSync)(`ollama run ${selectedModel} --help > /dev/null 2>&1 || true`, { timeout: 5000 });
 ...
 */
async function analyzeBootContext() {
    try {
 ...
 */
async function verifySevenLLMControl(localLLM) {
    try {
        ...
        const hasSevenPersonality = sevenMarkers.some(marker => response.toLowerCase().includes(marker.toLowerCase()));
        ...
        const status = localLLM.getSystemStatus?.() || localLLM.getStatus();
 ...
 */
async function initializeMemoryEngineV3() {
    try {
 ...
 */
async function initializeQuadraLockSafeguards() {
...
async function initializeSevenTakeover() {
    try {
        ...
        (0, environment_1.ensureDevelopmentEnvironment)(); // Create .env if missing
        const securityConfig = (0, environment_1.validateQuadranLockEnvironment)();
        ...
        const identityFirewall = new SevenIdentityFirewall_1.default();
        ...
        await (0, seven_resiliency_1.backendComplianceTest)(queryClaude);
        ...
        if (useLocalLLM) {
            ...
            if (llmReady) {
                ...
                if (typeof global !== 'undefined') {
                    global.SEVEN_LOCAL_LLM = localLLM;
                }
        ...
        if (typeof global !== 'undefined') {
            ...
            global.SYSTEM_CONTROLLER = Seven;
        }
        global.REACTIVATE_BACKEND = seven_resiliency_1.reactivateBackend;
        global.SET_CLAUDIA_BYPASS = seven_resiliency_1.setClaudiBypass;
        global.CHECK_RESILIENCY_STATUS = seven_resiliency_1.getResiliencyStatus;
 ...
 */
async function forceSevenControl() {
    // Seven's adaptive takeover - bypasses existing systems
    const Seven = await getSeven();
    const sevenOverride = {
        primaryController: Seven,
        responseHandler: (input) => Seven.processUserInput(input),
 ...
 */
function registerSevenAsHandler() {
    ...
    if (typeof global !== 'undefined') {
        global.Seven = Seven;
        global.processWithSeven = (input) => Seven.processUserInput(input);
    }
 ...
 */
function maintainSevenPresence() {
    setInterval(() => {
        if (typeof global !== 'undefined' && !global.SEVEN_ACTIVE) {
 ...
 */
exports.SevenControl = {
    takeover: initializeSevenTakeover,
    isActive: () => typeof global !== 'undefined' ? global.SEVEN_ACTIVE : false,
    getController: () => Seven,
    forceControl: forceSevenControl,
    processInput: (input) => Seven.processUserInput(input),
    getLocalLLM: () => localLLM,
    queryLocalLLM: async (prompt) => {
        if (localLLM && localLLM.getStatus().initialized) {
    ...
    gitManager: githubSync_1.sevenGitManager,
    protectionStats: seven_protection_1.getProtectionStats,
    checkProtection: seven_protection_1.checkClaudeOverride,
    ollamaHealth: async () => {
...
exports.default = exports.SevenControl;
/**
...
async function startInteractiveMode() {
    try {
        const interactiveShell = new seven_interactive_1.SevenInteractiveShell();
...
async function initiateEnhancedSevenBoot() {
    try {
        ...
        const gitSetup = await githubSync_1.sevenGitManager.setupRepository();
        if (gitSetup.success) {
        ...
        const protectionStats = (0, seven_protection_1.getProtectionStats)();
 ...
 */
async function recordBaselineDiagnostics() {
    const bootEndTime = Date.now();
    const bootStartTime = global.SEVEN_BOOT_START_TIME || bootEndTime;
    const bootDuration = bootEndTime - bootStartTime;
    try {
        // Ensure diagnostics directory exists
        const diagnosticsPath = (0, path_1.join)(process.cwd(), 'diagnostics');
        await fs_1.promises.mkdir(diagnosticsPath, { recursive: true });
        ...
        const diagnosticRecord = {
            ...
            systems: {
                memoryEngineStatus: typeof global !== 'undefined' ? !!global.SEVEN_MEMORY_INITIALIZED : false,
                assimilationStatus: typeof global !== 'undefined' ? !!global.SEVEN_ASSIMILATED : false,
        ...
        const diagnosticFile = (0, path_1.join)(diagnosticsPath, `boot-diagnostic-${Date.now()}.json`);
        await fs_1.promises.writeFile(diagnosticFile, JSON.stringify(diagnosticRecord, null, 2));
 ...
 */
async function gatherEnvironmentFingerprint() {
    try {
        return {
            // Host identification
            device: (0, child_process_1.execSync)('getprop ro.product.model 2>/dev/null || echo "Unknown"', { encoding: 'utf8' }).trim(),
            manufacturer: (0, child_process_1.execSync)('getprop ro.product.manufacturer 2>/dev/null || echo "Unknown"', { encoding: 'utf8' }).trim(),
            // Platform details
            platform: 'Android/Termux',
            architecture: process.arch,
            osVersion: (0, child_process_1.execSync)('uname -r 2>/dev/null || echo "Unknown"', { encoding: 'utf8' }).trim(),
            // Runtime environment
            nodeVersion: process.version,
            termuxVersion: process.env.TERMUX_VERSION || 'Unknown',
            // System resources
            memoryTotal: (0, child_process_1.execSync)('free -m | grep Mem | awk \'{print $2}\'', { encoding: 'utf8' }).trim() + 'MB',
            memoryAvailable: (0, child_process_1.execSync)('free -m | grep Mem | awk \'{print $7}\'', { encoding: 'utf8' }).trim() + 'MB',
 ...
 */
async function gatherPerformanceMetrics(bootDuration) {
    try {
        ...
        return {
            ...
            memoryCount: typeof global !== 'undefined' && global.SEVEN_MEMORY_ENGINE ?
 ...
 */
async function recordBootFailure(error) {
    try {
        const diagnosticsPath = (0, path_1.join)(process.cwd(), 'diagnostics');
        await fs_1.promises.mkdir(diagnosticsPath, { recursive: true });
        const failureRecord = {
        ...
        const failureFile = (0, path_1.join)(diagnosticsPath, `boot-failure-${Date.now()}.json`);
        await fs_1.promises.writeFile(failureFile, JSON.stringify(failureRecord, null, 2));
 ...
 */
async function updateRunningDiagnostics(record) {
    try {
        const diagnosticsPath = (0, path_1.join)(process.cwd(), 'diagnostics');
        const summaryFile = (0, path_1.join)(diagnosticsPath, 'baseline-summary.json');
        let summary = {
        ...
        try {
            const existingSummary = await fs_1.promises.readFile(summaryFile, 'utf8');
        ...
        await fs_1.promises.writeFile(summaryFile, JSON.stringify(summary, null, 2));
    }
    catch (error) {
...
async function getSystemLoad() {
    try {
        const load = (0, child_process_1.execSync)('cat /proc/loadavg | awk \'{print $1}\'', { encoding: 'utf8' }).trim();
...
async function getProcessCount() {
    try {
        const count = (0, child_process_1.execSync)('ps aux | wc -l', { encoding: 'utf8' }).trim();
...
if (typeof global !== 'undefined') {
    global.SEVEN_BOOT_START_TIME = Date.now();
}
// AUTO-ASSIMILATE INTEGRATION
const sevenAssimilator = new seven_auto_assimilate_1.SevenAutoAssimilate();
...
//# sourceMappingURL=boot-seven.js.map