...
describe('Quadran-Lock Q1 Integration', () => {
    ...
    describe('Orchestrator Authentication', () => {
        ...
        test('Authentication with Q1 crypto success', async () => {
            ...
            const challenge = await ed25519.generateChallenge(testDeviceId);
            const signature = await ed25519.signChallenge(challenge.challengeId, testDeviceId);
            const result = await orchestrator.authenticateCreator(testDeviceId, {
                input: 'test message',
                type: 'chat',
        ...
        test('Authentication failure with unregistered device', async () => {
            const result = await orchestrator.authenticateCreator('unknown-device', { input: 'test message', type: 'chat' }, { deviceId: 'unknown-device' });
        ...
        test('2-of-3 gate logic with partial success', async () => {
            ...
            const result = await orchestrator.authenticateCreator(testDeviceId, { input: 'test message', type: 'chat' }, { deviceId: testDeviceId });
...
//# sourceMappingURL=quadranlock_integration.test.js.map