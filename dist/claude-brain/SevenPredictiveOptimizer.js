...
exports.SevenPredictiveOptimizer = void 0;
...
class SevenPredictiveOptimizer extends events_1.EventEmitter {
    performanceAnalyzer;
    tacticalFallback;
    learningModel;
    isActive = false;
    ...
    constructor(performanceAnalyzer, tacticalFallback, baseDir) {
        super();
        ...
        this.patternsPath = (0, path_1.join)(base, 'predictive-patterns');
        this.learningModel = {
            patterns: new Map(),
    ...
    async initializePredictiveSystem() {
        ...
        try {
            ...
            await fs_1.promises.mkdir(this.patternsPath, { recursive: true });
     ...
     */
    async recordUsagePattern(context, actions, outcome) {
        if (!this.isActive)
            return;
        const pattern = {
     ...
     */
    async generatePredictiveOptimization(currentContext) {
        if (!this.isActive)
            return null;
    ...
    async analyzePatternsForPrediction(context) {
    ...
    predictPreloadActions(patterns, context) {
        const actions = [];
    ...
    calculatePredictionConfidence(patterns, context) {
        ...
        // Recency confidence
        const recentPatterns = patterns.filter(p => new Date(p.timestamp) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // Last 7 days
        );
        confidence += (recentPatterns.length / patterns.length) * 0.1;
        return Math.min(Math.max(confidence, 0.1), 0.95);
    }
    generatePredictionReasoning(patterns, context) {
        const reasons = [];
    ...
    generateBaselinePrediction(context) {
        return {
            confidence: 0.3,
            prediction: {
    ...
    async loadLearningModel() {
        try {
            const modelPath = (0, path_1.join)(this.patternsPath, 'learning-model.json');
            const data = await fs_1.promises.readFile(modelPath, 'utf8');
    ...
    async saveLearningModel() {
        try {
            ...
            await fs_1.promises.writeFile(modelPath, JSON.stringify(serializable, null, 2));
        }
        catch (error) {
     ...
     * PUBLIC API METHODS
     */
    isPredictiveOptimizationActive() {
        return this.isActive;
    }
    async getLearningStats() {
        let totalPatterns = 0;
        const patternsByTaskType = {};
        for (const [taskType, patterns] of this.learningModel.patterns.entries()) {
...
exports.SevenPredictiveOptimizer = SevenPredictiveOptimizer;
exports.default = SevenPredictiveOptimizer;
//# sourceMappingURL=SevenPredictiveOptimizer.js.map