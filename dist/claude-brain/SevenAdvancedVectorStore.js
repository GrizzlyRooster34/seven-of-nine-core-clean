...
exports.SevenAdvancedVectorStore = void 0;
const events_1 = require("events");
const path_1 = require("path");
const fs_1 = require("fs");
const SevenVectorStore_1 = require("./SevenVectorStore");
const SevenTacticalFallback_1 = __importDefault(require("./SevenTacticalFallback"));
class SevenAdvancedVectorStore extends events_1.EventEmitter {
    config;
    embeddings = new Map();
    clusters = new Map();
    correlations = [];
    ...
    isInitialized = false;
    storePath;
    chromaClient = null; // Will be initialized if ChromaDB available
    constructor(config, tacticalFallback, baseDir) {
        super();
        this.config = {
        ...
        this.fallbackStore = new SevenVectorStore_1.SevenVectorStore(baseDir); // Phase 1 fallback
        const base = baseDir || process.cwd();
        this.storePath = (0, path_1.join)(base, 'advanced-vector-store');
        this.initializeAdvancedVectorStore();
    }
    async initializeAdvancedVectorStore() {
        console.log('🚀 Seven Advanced Vector Store: Initializing enhanced semantic memory system...');
        try {
            // Ensure storage directory exists
            await fs_1.promises.mkdir(this.storePath, { recursive: true });
     ...
     */
    async storeAdvancedEmbedding(memoryId, content, importance, tags, metadata) {
    ...
    async generateAdvancedEmbedding(content) {
     ...
     */
    async searchAdvancedSemantic(queryText, options = {}) {
        if (this.config.fallbackMode) {
            const fallbackResults = await this.fallbackStore.searchSimilar(queryText, options.topK || 5, options.importanceFilter || 5);
            return {
                results: fallbackResults.map(result => ({
                    embeddingId: result.id || result.embeddingId,
                    ...
                    metadata: {
                        ...
                        emotionalContext: result.consciousnessContext || result.emotionalContext,
        ...
        try {
            ...
            for (const [id, embedding] of this.embeddings.entries()) {
                // Apply filters
                if (options.importanceFilter && embedding.metadata.importance < options.importanceFilter)
                    continue;
                if (options.trustLevelFilter && embedding.metadata.trustLevel < options.trustLevelFilter)
                    continue;
                if (options.conversationFilter && embedding.metadata.conversationId !== options.conversationFilter)
                    continue;
     ...
     */
    async updateClusters(newEmbedding) {
        try {
    ...
    async updateClusterCentroid(cluster) {
    ...
    generateClusterTopic(embedding) {
    ...
    calculateClusterCoherence(cluster, members) {
        if (members.length < 2)
            return 1.0;
        ...
        for (let i = 0; i < members.length; i++) {
            for (let j = i + 1; j < members.length; j++) {
                totalSimilarity += this.calculateCosineSimilarity(members[i].embedding, members[j].embedding);
                comparisons++;
            }
        }
     ...
     */
    async analyzeCrossConversationCorrelations(newEmbedding) {
        if (!newEmbedding.metadata.conversationId)
            return;
        try {
            ...
            for (const [id, embedding] of this.embeddings.entries()) {
                if (embedding.metadata.conversationId === currentConversationId)
                    continue;
                if (!embedding.metadata.conversationId)
                    continue;
                const similarity = this.calculateCosineSimilarity(newEmbedding.embedding, embedding.embedding);
                if (similarity > similarityThreshold) {
                    // Check if correlation already exists
                    const existing = this.correlations.find(corr => (corr.conversationId1 === currentConversationId && corr.conversationId2 === embedding.metadata.conversationId) ||
    ...
    extractSharedConcepts(emb1, emb2) {
        ...
        return concepts1.filter(concept => concepts2.includes(concept) && concept.length > 3).slice(0, 5);
    }
    determineRelationshipType(emb1, emb2) {
    ...
    async loadStorageData() {
        try {
            // Load embeddings
            const embeddingsPath = (0, path_1.join)(this.storePath, 'embeddings.json');
            try {
                const embeddingsData = await fs_1.promises.readFile(embeddingsPath, 'utf8');
            ...
            // Load clusters
            const clustersPath = (0, path_1.join)(this.storePath, 'clusters.json');
            try {
                const clustersData = await fs_1.promises.readFile(clustersPath, 'utf8');
            ...
            // Load correlations
            const correlationsPath = (0, path_1.join)(this.storePath, 'correlations.json');
            try {
                const correlationsData = await fs_1.promises.readFile(correlationsPath, 'utf8');
    ...
    async persistEmbedding(embedding) {
    ...
    async persistAllData() {
        try {
            ...
            await fs_1.promises.writeFile((0, path_1.join)(this.storePath, 'embeddings.json'), JSON.stringify(embeddingsArray, null, 2));
            // Persist clusters
            const clustersArray = Array.from(this.clusters.values());
            await fs_1.promises.writeFile((0, path_1.join)(this.storePath, 'clusters.json'), JSON.stringify(clustersArray, null, 2));
            // Persist correlations
            await fs_1.promises.writeFile((0, path_1.join)(this.storePath, 'correlations.json'), JSON.stringify(this.correlations, null, 2));
        }
        catch (error) {
    ...
    async getCorrelations(conversationId) {
        if (conversationId) {
            return this.correlations.filter(corr => corr.conversationId1 === conversationId || corr.conversationId2 === conversationId);
        }
...
exports.SevenAdvancedVectorStore = SevenAdvancedVectorStore;
exports.default = SevenAdvancedVectorStore;
//# sourceMappingURL=SevenAdvancedVectorStore.js.map