...
exports.MobileOptimizationTriggers = void 0;
const events_1 = require("events");
const child_process_1 = require("child_process");
const fs_1 = require("fs");
const util_1 = require("util");
const PerformanceAnalyzer_1 = __importDefault(require("./PerformanceAnalyzer"));
class MobileOptimizationTriggers extends events_1.EventEmitter {
    ...
    constructor(performanceAnalyzer) {
        super();
    ...
    async getThermalState() {
        try {
            ...
            if (resourceStatus && resourceStatus.cpuUsage > 90)
                return 'critical';
            if (resourceStatus && resourceStatus.cpuUsage > 70)
                return 'hot';
            if (resourceStatus && resourceStatus.cpuUsage > 50)
                return 'warm';
            return 'normal';
        }
        catch {
    ...
    async getMemoryPressure() {
        try {
            ...
            if (!resourceStatus)
                return 'low';
            if (resourceStatus.memoryUsage > 90)
                return 'critical';
            if (resourceStatus.memoryUsage > 75)
                return 'high';
            if (resourceStatus.memoryUsage > 50)
                return 'medium';
            return 'low';
        }
        catch {
    ...
    async getAvailableStorage() {
        try {
            ...
            const stats = await fs_1.promises.statfs(process.cwd());
    ...
    hasDeviceStateChanged(newState) {
        if (!this.deviceState)
            return true;
     ...
     */
    evaluateAndApplyOptimization(trigger) {
        if (!this.deviceState)
            return;
    ...
    applyOptimizationProfile(profileName, trigger) {
    ...
    getOptimizationReason(profileName) {
        if (!this.deviceState)
            return 'Unknown';
        const state = this.deviceState;
        const reasons = [];
        ...
        if (state.memoryPressure === 'critical')
            reasons.push('critical memory');
        else if (state.memoryPressure === 'high')
    ...
    forceProfileSwitch(profileName) {
...
exports.MobileOptimizationTriggers = MobileOptimizationTriggers;
exports.default = MobileOptimizationTriggers;
//# sourceMappingURL=MobileOptimizationTriggers.js.map