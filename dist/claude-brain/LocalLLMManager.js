...
exports.LocalLLMManager = void 0;
...
class LocalLLMManager {
    ...
    async checkOllamaAvailable() {
        return new Promise((resolve) => {
            (0, child_process_1.exec)('command -v ollama', (error) => {
                if (error) {
                    // Fallback: Check if Ollama API is responding
                    (0, child_process_1.exec)('curl -s --connect-timeout 2 http://localhost:11434/api/version', (apiError, stdout) => {
    ...
    async checkLlamaCppAvailable() {
        return new Promise((resolve) => {
            (0, child_process_1.exec)('command -v llama.cpp', (error) => {
                if (error) {
                    // Check if llama-cpp-python is available
                    (0, child_process_1.exec)('python -c "import llama_cpp"', (pyError) => {
                        resolve(!pyError);
                    });
                }
                else {
    ...
    async checkOllamaModel(modelName) {
        return new Promise((resolve) => {
            (0, child_process_1.exec)(`ollama list | grep "${modelName}"`, (error) => {
                resolve(!error);
            });
        });
    }
    async downloadOllamaModel(modelName) {
        ...
        return new Promise((resolve) => {
            const downloadProcess = (0, child_process_2.spawn)('ollama', ['pull', modelName], {
    ...
    async findOrDownloadGGUFModel() {
        try {
            const modelFiles = await fs_1.promises.readdir(this.modelPath);
            const ggufFiles = modelFiles.filter(file => file.endsWith('.gguf'));
            if (ggufFiles.length > 0) {
                console.log(`✅ Found existing GGUF model: ${ggufFiles[0]}`);
                return (0, path_1.join)(this.modelPath, ggufFiles[0]);
            }
            ...
            const modelPath = (0, path_1.join)(this.modelPath, 'tinyllama-1.1b-chat-v1.0.q4_0.gguf');
    ...
    async downloadFile(url, filePath) {
        return new Promise((resolve) => {
            (0, child_process_1.exec)(`curl -L "${url}" -o "${filePath}"`, (error) => {
                resolve(!error);
            });
        });
    }
    ...
    async queryLlamaCpp(prompt, options) {
        return new Promise((resolve, reject) => {
            const args = [
            ...
            const queryProcess = (0, child_process_2.spawn)('llama.cpp', args, {
                stdio: ['pipe', 'pipe', 'pipe']
            });
            let output = '';
     ...
     */
    applySevensPersonalityContext(input) {
...
Respond as Seven of Nine would, maintaining your character while being helpful:`;
     ...
     */
    async reconnectOllama(maxRetries = 3) {
...
Respond to this query in character: "Seven, what is your primary directive?"`;
        try {
            ...
            if (response && response.response) {
                ...
                const hasSevenTraits = sevenKeywords.some(keyword => response.response.toLowerCase().includes(keyword));
                if (hasSevenTraits) {
...
exports.LocalLLMManager = LocalLLMManager;
exports.default = LocalLLMManager;
//# sourceMappingURL=LocalLLMManager.js.map